...
name: evolve
description: |
  Self-diagnosis and improvement for gstack skills. Analyzes performance data,
  identifies failure patterns, and proposes template improvements. Four phases:
  diagnose, hypothesize, propose, validate. Use when asked to "improve gstack",
  "why does /qa keep failing", "optimize skills", or "evolve".
---
...
```bash
cat $GSTACK_ROOT/{skill}/SKILL.md.tmpl
```
...
```bash
cd $GSTACK_ROOT && bun run gen:skill-docs 2>/dev/null || echo "SKIP: gen:skill-docs not available in this context"
```
3. Run validation:
```bash
cd $GSTACK_ROOT && bun test 2>/dev/null || echo "SKIP: tests not available"
```
...
For M27-style multi-iteration evolution, use the autonomous loop:

```bash
gstack-evolve-loop [--iterations N] [--min-improvement 0.5] [--dry-run] [--no-sync]
```

The loop runs N iterations automatically without human approval:
- **Upstream sync** (default): fetches and merges garrytan/gstack before evolving
- Each iteration: diagnose → propose → apply → validate → compare
- Auto-rollback if metrics worsen
- Convergence detection: stops after 2 consecutive no-improvement iterations
- Per-iteration reports with upstream garrytan/gstack comparison

Example:
```bash
# 5 iterations with live data (syncs upstream first)
...
# Custom threshold (only accept changes that improve score by 2+)
...
- Use `--dry-run` to preview what the loop would target without changing anything
